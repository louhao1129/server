...
# 设置配置  
BaiduPCS-Go config set -max_parallel 150 -savedir ~/Downloads
```
...
BaiduPCS-Go upload --norapid <本地文件/目录的路径1> <文件/目录2> <文件/目录3> ... <目标目录> 
BaiduPCS-Go u --norapid <本地文件/目录的路径1> <文件/目录2> <文件/目录3> ... <目标目录>  
```
...
最后，上传或者下载的过程耗时可能很久，推荐挂在后台，防止因为连接断开导致上传、下载失败，可以使用screen，tmux、zellij等工具实现此目的